...
export function BarInspector({ obj }: { obj: AnyObj }) {
  const bar: any = obj as any;
  ...
  const colorHex = normalizeHex(bar?.style?.color ?? "#3EA3FF");
  const alpha = clampInt(bar?.style?.alpha, 100);

  const bgHex = normalizeHex(bar?.style?.backgroundColor ?? "#3EA3FF");
  const bgAlpha = clampInt(bar?.style?.backgroundAlpha, 40);

  return (
    <div className="insRoot">
      <div className="insTypeBar">Bar</div>

      <Label>Name</Label>
      <TextField value={bar?.name ?? "Bar"} onChange={(e) => Actions.updateObject(obj.id, { name: e.target.value })} />

      <Collapse title="Transform" open={openTransform} onToggle={() => setOpenTransform((v) => !v)}>
        <Row2>
          <div>
            <Label>Position X</Label>
            <SpinNumber value={bar.transform?.x ?? 0} onChange={(v) => Actions.updateObjectDeep(obj.id, ["transform", "x"], v)} />
          </div>
          <div>
            <Label>Position Y</Label>
            <SpinNumber value={bar.transform?.y ?? 0} onChange={(v) => Actions.updateObjectDeep(obj.id, ["transform", "y"], v)} />
          </div>
        </Row2>
      ...
      <Collapse title="Settings" open={openSettings} onToggle={() => setOpenSettings((v) => !v)}>
        ...
        <SpinNumber
          value={bar.settings?.previewValue ?? 50}
          min={0}
          max={100}
        ...
        <div style={{ marginTop: 12 }}>
          <Row2>
            ...
            <div>
              <Label>Radius</Label>
              <SpinNumber
                value={bar.style?.radius ?? 0}
                min={0}
                max={2000}
                onChange={(v) => Actions.updateObjectDeep(obj.id, ["style", "radius"], v)}
              />
            </div>
          </Row2>
        </div>
        ...
        <div style={{ marginTop: 12 }}>
          <Row2>
            ...
            <div>
              ...
              <SpinNumber
                value={bar.style?.backgroundGlow ?? 0}
                min={0}
                max={1000}
